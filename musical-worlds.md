...
Scenery and world pieces are ordinary named nodes in the same GLB — the distinction lives entirely in who asks for them — so a model can be both. The harbour's quay is one shed collected as a world piece plus two more of it, smaller, placed as scenery; its jetty is one pier section collected plus two more laid end to end, which is also the fix for a prop whose model is a _section_ of a thing rather than the whole thing.

**Scenery can be the socket a world piece lands in.** The harbour's chest sits inside a dug pit, and the pit is scenery: it is there from beat 0, empty, for the whole run. That's worth more than making the pit part of the chest model, because the island now advertises a specific absence — an empty hole reads as "something goes here" far more strongly than a ghost does, and it costs one line. Where a piece has an obvious socket (a pit, a plinth, a mooring, a foundation), place the socket as scenery and let the catch fill it.
...
`packScale` is that rule taken to its conclusion: don't normalize per prop at all, multiply the source by one factor for the whole record, and let the artist's proportions through untouched. The city is one number — 0.142, which lands Kay's tallest building at 0.42 on the label — and the hydrant, the bench and the fire escape come out wherever he put them. Twenty-one hand-chosen sizes are twenty-one chances to disagree with a pack that already agrees with itself. Use it on a record built from one pack in one pass; keep per-prop sizing where composition has to win, which is what the other two records do.

**The pack decides how far the code can reach into a prop.** Kenney's city kit gave every building a `window` material, so the night repaint could darken the walls and put an emissive in the glass. KayKit's city samples ONE material — `citybits_texture` — for every building, car and bin it ships, and there is no repaint that finds the panes without also finding the parapet. Two things recover from that. `recolor` runs per source file, before the merge, so each prop still owns its own copy of that material and the night can be mixed **per prop** even when it can't be mixed per surface — which is the more useful axis anyway, since a city at night is dark buildings with a few bright things in front of them. And the windows come back in `neonDressing.ts` by **shape**: weld a mesh's triangles into panels, and a window pane is a flat quad 0.16 × 0.30 in the pack's units where the wall it sits in is a single 2.0 × 2.85 shell. The geometry knows what the material doesn't.

**A prop with one decorated face is a rotation problem, not a placement problem.** Every KayKit city building puts its windows, awnings and doors on `+z` and leaves the other three sides blank brick. Rotating each one to face its street — what a real block does — pointed every facade at the middle of the island, because that is where the roads are, and the near half of the plate went blank. They face **outward** instead, snapped to the nearest of the six hex edge normals, so the half closest to the camera is always the lit half whatever angle the disc has turned to. The street plan is carried by the glow on the tile rims, which was doing it better than a row of doors anyway.

**Two things the middle tile is load-bearing for.** A hill tile sits 0.07 proud, and that is exactly what buries the record's spindle — a 0.09 post that otherwise stands 0.026 above a path tile. Every island puts a hill and its tallest prop on `0,0`; the neon layout ran a street through it once and got a chrome puck in the middle of a junction. And a building fills its tile (0.285 of a 0.322 hex), so there is no room beside it for a lamp: kerbside props stand on the **road** tile pushed 0.13 toward the pavement, which is the same kerb line reached from the empty side.
