...
- Runner: the Rogue wearing the Ranger's head, both from the [KayKit Adventurers Character Pack](https://kaylousberg.itch.io/kaykit-adventurers) by Kay Lousberg — CC0 (reworked: cape dropped, headphones added, run/idle/cheer clips from the same pack's 1.x rig, see `scripts/build-runner.mjs`)
